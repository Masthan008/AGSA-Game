...
app.get('/api/v1/me/tree-assignments', async (req, res) => {
    const studentId = requestOwnerId(req);
    if (!studentId)
        return apiError(req, res, 401, 'AUTH_REQUIRED', 'Authentication required');
...
app.post('/api/v1/me/lesson-checkpoints', validateBody(treeCheckpointSchema), async (req, res) => {
    const userId = requestOwnerId(req);
    if (!userId)
        return apiError(req, res, 401, 'AUTH_REQUIRED', 'Authentication required');
...
app.post('/api/v1/me/tree-assignments/:id/submit', validateBody(treeSubmissionSchema), async (req, res) => {
    ...
    if (!assignment)
        return apiError(req, res, 404, 'ASSIGNMENT_NOT_FOUND', 'Tree assignment not found');
    if (assignment.submissions.length >= assignment.maxAttempts)
        return apiError(req, res, 409, 'ATTEMPT_LIMIT', 'Maximum attempts reached');
    ...
    const response = await prisma.$transaction(async (tx) => {
        ...
        const submission = await tx.treeSubmission.create({ data: { assignmentId: assignment.id, studentId, status: completed ? 'completed' : 'needs_retry', finalState: req.body.finalState, score: adjustedScore, hintsUsed: req.body.hintsUsed, durationMs: req.body.durationMs, attemptNumber: assignment.submissions.length + 1, feedback: assessment, submittedAt: new Date(), steps: { create: req.body.steps.map((step, sequence) => ({ ...step, sequence, correct: assessment.skills[step.skillKey || ''] ?? null })) } } });
        ...
        if (completed && !priorCompleted)
            await tx.user.update({ where: { id: studentId }, data: { xp: { increment: assignment.xpReward } } });
        const result = { submission, assessment, xpAwarded: completed && !priorCompleted ? assignment.xpReward : 0 };
        await tx.idempotencyRecord.create({ data: { userId: studentId, key, operation: 'tree-assignment-submit', responseJson: result, expiresAt: new Date(Date.now() + 7 * 86400000) } });
        return result;
    });
    res.status(201).json(response);
});
app.post('/api/v1/teacher/tree-assignments', validateBody(treeAssignmentSchema), async (req, res) => {
    if (!(await adminGate(req, res)))
        return;
    const teacherId = requestOwnerId(req);
    if (req.body.classroomId) {
        ...
        if (!classroom)
            return apiError(req, res, 403, 'CLASSROOM_SCOPE_REQUIRED', 'Student is not in this teacher classroom');
    }
    const assignment = await prisma.treeAssignment.create({ data: { ...req.body, teacherId, dueAt: req.body.dueAt ? new Date(req.body.dueAt) : null, initialState: req.body.initialState, targetState: req.body.targetState } });
...
app.get('/api/v1/teacher/students/:id/weak-skills', async (req, res) => {
    if (!(await adminGate(req, res)))
        return;
    const teacherId = requestOwnerId(req);
...
app.get('/api/v1/teacher/submissions/:id/replay', async (req, res) => {
    if (!(await adminGate(req, res)))
        return;
    const teacherId = requestOwnerId(req);
    ...
    if (!submission)
        return apiError(req, res, 404, 'SUBMISSION_NOT_FOUND', 'Submission not found');
