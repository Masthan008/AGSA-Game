...
const TREE_CURRICULUM_QUIZZES: Record<number, Array<[string,string[],number,string]>> = {
  1:[['Which two rules must every AVL tree satisfy?',['BST ordering and balance factor in −1…+1','Complete-tree shape and red root','Every leaf equal value','Exactly two children'],0,'AVL combines BST ordering with height balance.'],['A node has left height 3 and right height 1. Its balance factor is:',['+2','−2','+1','0'],0,'BF = left height − right height = 2.'],['Which tree is definitely not AVL?',['A BST containing a node with BF −2','A BST with every BF 0','An empty tree','A one-node tree'],0,'Any balance factor outside −1…+1 violates AVL.']],
  2:[['AVL search differs from ordinary BST search because:',['It does not; it follows the same comparisons','It rotates on every comparison','It scans every node','It uses hashing'],0,'Balance changes height, not the BST search rule.'],['A leaf height is 1 when an empty subtree height is:',['0','1','−2','Undefined'],0,'This common convention makes leaf height 1.'],['Why track heights after updates?',['To locate the first unbalanced ancestor','To sort keys again','To count colors','To choose disk blocks'],0,'Updated heights reveal imbalance on the return path.']],
  3:[['The sequence 30,20,10 creates which case?',['LL','RR','LR','RL'],0,'The heavy path is left-left.'],['An RR imbalance is repaired with:',['A left rotation','A right rotation','Two right rotations','No rotation'],0,'The right child rises through a left rotation.'],['Single rotations apply when parent and heavy child lean:',['In the same direction','In opposite directions','Neither direction','Only toward the root'],0,'LL and RR are same-direction cases.']],
  4:[['30,10,20 produces:',['LR','LL','RR','RL'],0,'The root leans left while its child leans right.'],['An RL repair performs:',['Right rotation on child, then left on parent','Two left rotations','One right rotation','Left on child, right on parent'],0,'Repair the child direction first, then the parent.'],['Why are double rotations necessary?',['The inner grandchild must become subtree root','They increase height','They remove two keys','They change BST ordering'],0,'The inner grandchild is correctly positioned by two rotations.']],
  5:[['AVL insertion first behaves like:',['BST insertion','Heap insertion','BFS','Hash probing'],0,'Insert using BST order, then rebalance upward.'],['After insertion, ancestors are checked in what order?',['From inserted node back toward root','Root to every leaf','Random order','Level order only'],0,'Only the insertion path can have changed heights.'],['A rotation must preserve:',['Inorder key order','Original root','All node heights','Insertion order'],0,'Rotations restructure without changing sorted order.']],
  6:[['AVL deletion may require:',['Rebalancing multiple ancestors','At most one comparison','No height updates','A node split'],0,'Height reduction can propagate to the root.'],['Deleting a two-child BST node normally uses:',['Inorder predecessor or successor','Any leaf','The root always','A random key'],0,'Replacement preserves BST ordering.'],['After deletion, BF +2 with left child BF 0 uses:',['Right rotation','Left rotation','RL rotation','No repair'],0,'A left-heavy node with nonnegative left child takes a right rotation.']],
  7:[['In a B-tree, all leaves are:',['At the same depth','Binary','Always full','Linked'],0,'Uniform leaf depth is a defining invariant.'],['A degree-2 B-tree node holds at most:',['3 keys','2 keys','4 keys','1 key'],0,'Maximum keys = 2t−1 = 3.'],['Internal children normally equal:',['Keys + 1','Keys − 1','Twice keys','One'],0,'Separator keys divide the child ranges.']],
  8:[['B-tree search selects a child using:',['Separator key ranges','Balance factors','Node colors','Heap indices'],0,'Keys partition values into ordered ranges.'],['Insertion finishes initially at:',['A leaf','The root only','Any internal node','A sibling'],0,'Search descends to the target leaf.'],['Before descending into a full child, a standard insertion can:',['Split it','Delete it','Rotate the root','Ignore capacity'],0,'Top-down splitting avoids descending into a full node.']],
  9:[['When a B-tree node splits, which key moves upward?',['The median','The minimum','The maximum','Every key'],0,'Median promotion separates the two resulting nodes.'],['A root split causes tree height to:',['Increase by one','Decrease by one','Become zero','Double'],0,'A new root is created above two children.'],['Cascading splits move:',['Toward the root','Only toward leaves','Sideways only','Outside the tree'],0,'Parent overflow can continue upward.']],
  10:[['Before deleting from a minimum-size child, try to:',['Borrow or merge','Split it','Add a random key','Rotate as AVL'],0,'Deletion prevents descending into an underfull child.'],['Borrowing moves a sibling key through:',['The parent separator','The root pointer','A hash function','A leaf link'],0,'The parent separator moves down while a sibling key replaces it.'],['If the root becomes empty with one child:',['That child becomes the new root','The tree is invalid forever','Split the child','Add a zero key'],0,'Root contraction reduces height while preserving invariants.']],
};

export function getQuizQuestionsForLevel(levelId?: string): QuizQuestion[] {
  if (!levelId) return QUIZ_QUESTIONS;
  const levelNumber=Number(levelId.match(/^level-(\d+)-/)?.[1]);
  if(levelNumber>=1&&levelNumber<=10)return TREE_CURRICULUM_QUIZZES[levelNumber].map(([question,options,correctAnswerIndex,explanation],index)=>({id:`tree-v4-l${levelNumber}-q${index+1}`,levelId,difficulty:(['beginner','medium','mastery'] as const)[index],xpReward:[20,30,50][index],question,options,correctAnswerIndex,explanation,hint:'Use the structural invariant introduced in this level.'}));
